...
final class RecognitionResultTests: XCTestCase {
    ...
    func testInitizationForCar() {
        ...
        XCTAssertEqual(result.recognition,
                       RecognitionResult.Recognition.car(firstCar),
                       "Recognition Result should return car for its label.")
    }
    ...
    func testInitializationForOtherCar() {
        ...
        guard let result = sut else { return }
        XCTAssertEqual(result.recognition,
                       RecognitionResult.Recognition.otherCar,
                       "Recognition Result should return otherCar enum type for label that's not in local data base.")
    }
    ...
    func testInitializationForNotCar() {
        ...
        guard let result = sut else { return }
        XCTAssertEqual(result.recognition,
                       RecognitionResult.Recognition.notCar,
                       "Recognition Result should return notCar enum type for label 'not car'.")
    }
    ...
    private func testResult(for label: String) {
        sut = RecognitionResult(label: label,
                                confidence: 0.9,
                                carsDataService: CarsDataService(localDataService: localCarsDataService))
