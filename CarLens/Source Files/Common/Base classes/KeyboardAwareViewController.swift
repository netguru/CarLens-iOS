...
internal class KeyboardAwareViewController: UIViewController {
    ...
    var keyboardWillHide: (_ animationDuration: TimeInterval) -> Void = { _ in }
    ...
    @objc private func keyboardWillShowSelector(notification: NSNotification) {
        ...
        let keyboardAnimation: TimeInterval? =
            (userInfo[UIResponder.keyboardAnimationDurationUserInfoKey] as? NSNumber)?.doubleValue
        guard let keyboardRect = keyboardFrame?.cgRectValue,
            let keyboardAnimationDuration = keyboardAnimation else { return }
