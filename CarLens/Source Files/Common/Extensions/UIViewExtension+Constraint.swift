...
internal extension UIView {
    ...
    @discardableResult func constraintToSuperviewEdges(excludingAnchors: [Anchor]? = nil,
    ...
    ///   - insets: Insets to use, .zero by default
    /// - Returns: Created constraints
    @discardableResult func constraintToEdges(of view: UIView,
        ...
        if let excludedAnchors = excludingAnchors {
            ...
            if !excludedAnchors.contains(.right) {
            ...
            if !excludedAnchors.contains(.left) {
    ...
    ///   - insets: Insets to use, .zero by default
    /// - Returns: Created constraints
    @discardableResult func constraintToSuperviewLayoutGuide(excludingAnchors: [Anchor]? = nil,
                                                             withInsets insets: UIEdgeInsets = .zero)
                                                             -> [NSLayoutConstraint] {
        guard let superview = superview else {
            fatalError("Cannot constrain to nil superview")
    ...
    ///   - insets: Insets to use, .zero by default
    /// - Returns: Created constraints
    @discardableResult func constraintToLayoutGuide(of view: UIView,
                                                    excludingAnchors: [Anchor]? = nil,
                                                    withInsets insets: UIEdgeInsets = .zero)
                                                    -> [NSLayoutConstraint] {
        var constraints = [NSLayoutConstraint]()
        if #available(iOS 11.0, *) {
            ...
            if let excludedAnchors = excludingAnchors {
                ...
                if !excludedAnchors.contains(.right) {
                    constraints.append(rightAnchor.constraint(equalTo: layoutGuide.rightAnchor,
    ...
    /// - Returns: Created constraints
    @discardableResult func constraintCenterToSuperview(axis: [NSLayoutConstraint.Axis] = [.horizontal, .vertical],
                                                        withConstant constant: CGPoint = .zero)
                                                        -> [NSLayoutConstraint] {
        guard let superview = superview else {
            fatalError("Cannot constrain to nil superview")
        }
    ...
    ///   - constant: Constant value to use for constraining
    /// - Returns: Created constraints
    @discardableResult func constraintCenter(to view: UIView,
                                             axis: [NSLayoutConstraint.Axis] = [.horizontal, .vertical],
                                             withConstant constant: CGPoint = .zero) -> [NSLayoutConstraint] {
        var constraints = [NSLayoutConstraint]()
        if axis.contains(.horizontal) {
            constraints.append(centerXAnchor.constraint(equalTo: view.centerXAnchor, constant: constant.x))
        }
        if axis.contains(.vertical) {
            constraints.append(centerYAnchor.constraint(equalTo: view.centerYAnchor, constant: constant.y))
        }
        NSLayoutConstraint.activate(constraints)
        return constraints
    }
